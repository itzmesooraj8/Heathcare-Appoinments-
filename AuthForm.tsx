...
const AuthForm = () => {
  ...
  const handleDemoLogin = async (role: string) => {
    ...
    try {
      const result = await apiService.demoLogin(role);
      if (result.data) {
        navigate('/dashboard');
      } else {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          ...
          {!isSignup && (
            <div className="mt-8 pt-6 border-t border-gray-200">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">Quick Demo Access:</h3>
              <div className="space-y-3">
                {['patient', 'doctor', 'admin'].map((role) => (
                  <button
                    ...
                    disabled={loading}
                    className="w-full text-left px-4 py-3 text-sm bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors border border-gray-200 group disabled:opacity-50"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="font-semibold capitalize text-gray-900">{role} Demo</span>
                        <p className="text-gray-600">Quick access to {role} dashboard</p>
                      </div>
                      {getRoleIcon(role)}
                    </div>
                  </button>
